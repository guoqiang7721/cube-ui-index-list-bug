...
export default class WxAudio {
  ...
  initAudioEvent() {
    ...
    ;(_this.wxAudio.onplaying = function() {
      ...
      _this.reduceTBefore =
        Date.parse(date) - Math.floor(_this.wxAudio.currentTime * 1000)
    }),
    ...
    (_this.wxAudio.onloadedmetadata = function() {
      ...
      // 初始化视频时间
      _this.wxAudioDuration.innerText = _this.formartTime(
        _this.wxAudio.duration
      )
    }),
    ...
    (_this.wxAudio.onprogress = function() {
      if (_this.wxAudio.buffered.length > 0) {
        ...
        for (var i = 0; i < _this.wxAudio.buffered.length; i++) {
          bufferedT +=
              _this.wxAudio.buffered.end(i) - _this.wxAudio.buffered.start(i)
      ...
      if (!_this.wxAudio.paused) {
        _this.reduceTAfter =
            Date.parse(date) - Math.floor(_this.currentT * 1000)
    ...
    ;(_this.wxAudio.ontimeupdate = function() {
      ...
      if (!_this.isDrag) {
        ...
        _this.currentP = Number(
          (_this.wxAudio.currentTime / _this.durationT) * 100
        )
        _this.reduceTBefore =
          Date.parse(date) - Math.floor(_this.currentT * 1000)
        ...
        // 更改时间进度
        _this.wxAudioCurrent.innerText = _this.formartTime(
          _this.wxAudio.currentTime
        )
        _this.showLoading(false)
      }
    }),
    ...
    _this.wxAudioOrigin.onmousedown = function(event) {
      ...
      _this.wxAudioC.onmousemove = function(event) {
        if (_this.isDrag) {
          var e = event || window.event
          var thisX = e.clientX
          _this.dragProgressTo = Math.min(
            _this.maxProgressWidth,
            Math.max(0, l + (thisX - x))
          )
          // update Time
      ...
      _this.wxAudioC.onmouseup = function() {
        if (_this.isDrag) {
          _this.isDrag = false
          _this.wxAudio.currentTime = Math.floor(
            (_this.dragProgressTo / _this.maxProgressWidth) * _this.durationT
          )
        } else {
        }
      }
