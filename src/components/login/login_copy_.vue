<template>
  <div class="container">
    <div class="header">
      ...
      />
      <p>用一张名片</p>
      <p v-if="extraProUserInfo && extraProUserInfo.bid !== '175'">
        开启{{ extraProUserInfo.brand }}的无限可能
      </p>
      <p v-else>
        开启的无限可能
      </p>
...
export default {
  ...
  activated() {
    const { brandName } = this.getParams()
  ...
  methods: {
    ...
    _createBtn() {
      ...
      if (!this.vBrandInfo.name && !brandId) {
        const popToastObj = {
          _this: this,
          txt: '请选择品牌'
        }
    ...
    create() {
      this.isLoading = true
      var _this = this
      const { brandId, nid } = this.getParams()
      this.$api.login
        .create({
          vid: _this.vid,
          token: _this.token,
          ...
          bid: brandId || _this.vBrandInfo.id,
